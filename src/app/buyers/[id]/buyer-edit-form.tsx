...
export default function BuyerEditForm({ data }: BuyerEditFormProps) {
  ...
  const onSubmit = async (formData: BuyerFormData) => {
    setLoading(true);
    try {
      const result = await updateBuyerAction(
        buyer.id,
        formData,
        new Date(buyer.updatedAt).toISOString()
      );
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update the buyer's details</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <input type="hidden" value={buyer.updatedAt} />
